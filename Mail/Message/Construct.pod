...
=item B<attach> =E<gt> BODY|MESSAGE|ARRAY-OF-BODY
...
=item B<data> =E<gt> STRING|ARRAY-OF-LINES
...
=item B<Subject> =E<gt> STRING|CODE
...
=item B<include> =E<gt> 'INLINE'|'ATTACH'
...
=item B<max_signature> =E<gt> INTEGER
...
=item B<signature> =E<gt> BODY|MESSAGE
...
=item B<strip_signature> =E<gt> REGEXP|STRING|CODE
...
=item B<group_reply> =E<gt> BOOLEAN
...
=item B<include> =E<gt> 'NO'|'INLINE'|'ATTACH'
...
=item B<postlude> =E<gt> BODY|LINES
...
=item B<prelude> =E<gt> BODY|LINES
