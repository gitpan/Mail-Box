...
Mail::Message::Body
  has extra code in Mail::Message::Body::Construct
  has extra code in Mail::Message::Body::Encode
...
=item B<based_on> =E<gt> BODY X<new(based_on)>
...
=item B<checked> =E<gt> BOOLEAN X<new(checked)>
...
=item B<data> =E<gt> ARRAY-OF-LINES | STRING X<new(data)>
...
=item B<disposition> =E<gt> STRING|FIELD X<new(disposition)>
...
=item B<eol> =E<gt> 'CR'|'LF'|'CRLF'|'NATIVE' X<new(eol)>
...
=item B<file> =E<gt> FILENAME|FILEHANDLE|IOHANDLE X<new(file)>
...
=item B<mime_type> =E<gt> STRING|FIELD|MIME X<new(mime_type)>
...
=item B<transfer_encoding> =E<gt> STRING|FIELD X<new(transfer_encoding)>
...
=item B<result_type> =E<gt> CLASS X<decoded(result_type)>
...
=item B<eol> ['CR'|'LF'|'CRLF'|'NATIVE'] X<eol>
...
=item B<transferEncoding> [STRING|FIELD] X<transferEncoding>
...
=item B<fileLocation> [BEGIN,END] X<fileLocation>
...
=item B<read> PARSER, HEAD, BODYTYPE [,CHARS [,LINES]] X<read>
