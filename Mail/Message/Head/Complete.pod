...
=item B<add> FIELD | LINE | (NAME,BODY[,ATTRS]) X<add>
...
=item B<removeField> FIELD X<removeField>
...
=item B<set> FIELD | LINE | (NAME, BODY [,ATTRS]) X<set>
...
=item B<addResentGroup> RESENT-GROUP|DATA X<addResentGroup>
...
=item B<resentGroups>  X<resentGroups>
...
=item B<messagIdPrefix> [STRING] X<messagIdPrefix>
