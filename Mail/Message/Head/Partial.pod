...
=head1 SYNOPSIS
 ...
 $partial->isa('L<Mail::Message::Head|Mail::Message::Head>')  # true
...
When many fields are removed, for instance with L<removeFields()|"Access to the header"> or
L<removeFieldsExcept()|"Access to the header">, then it is useful to remove the list of undefs
...
See also L<removeField()|"Access to the header"> which is used to remove one field object from
...
are explained in L<Mail::Message::Head::ResentGroup|Mail::Message::Head::ResentGroup>.  For removing single
groups (for instance because you want to keep the last), use
L<Mail::Message::Head::ResentGroup::delete()|"The header">.
