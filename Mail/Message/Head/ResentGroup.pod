...
=item B<new> [FIELDS], OPTIONS X<new>
...
=item B<Bcc> =E<gt> STRING|OBJECT|OBJECTS X<new(Bcc)>

=item B<Cc> =E<gt> STRING|OBJECT|OBJECTS X<new(Cc)>

=item B<Date> =E<gt> STRING X<new(Date)>
...
=item B<Delivered-To> =E<gt> STRING|FIELD X<new(Delivered-To)>

=item B<From> =E<gt> STRING|OBJECT|OBJECTS X<new(From)>

=item B<Message-ID> =E<gt> STRING|FIELD X<new(Message-ID)>

The C<Resent-Message-ID> which identifies this resent group.  The FIELD
must contain a message id.

=item B<Received> =E<gt> STRING X<new(Received)>

The C<Received> field is the starting line for a resent group of header
lines, therefore it is obligatory.

=item B<Return-Path> =E<gt> STRING|FIELD X<new(Return-Path)>

=item B<Sender> =E<gt> STRING|OBJECT X<new(Sender)>
...
=item B<To> =E<gt> STRING|OBJECT|OBJECTS X<new(To)>
...
=item B<dateTimestamp>  X<dateTimestamp>

The timestamp as stored within the C<Resent-Date> field converted to
local system time.

=item B<deliveredTo>  X<deliveredTo>
...
=item B<createReceived>  X<createReceived>
