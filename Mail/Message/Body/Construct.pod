...
=item B<attach> MESSAGES, OPTIONS X<attach>
...
=item B<concatenate> COMPONENTS X<concatenate>
...
=item B<foreachLine> CODE X<foreachLine>
...
=item B<max_lines> =E<gt> INTEGER|undef X<stripSignature(max_lines)>

The maximum number of lines which can be the length of a signature.
Specify C<undef> to remove the limit.

=item B<pattern> =E<gt> REGEX|STRING|CODE X<stripSignature(pattern)>
...
=item B<result_type> =E<gt> CLASS X<stripSignature(result_type)>
