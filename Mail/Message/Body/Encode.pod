...
=item B<addTransferEncHandler> NAME, CLASS|OBJECT X<addTransferEncHandler>
...
=item B<mime_type> =E<gt> STRING|FIELD X<encode(mime_type)>
...
=item B<result_type> =E<gt> CLASS X<encode(result_type)>
...
=item B<transfer_encoding> =E<gt> STRING|FIELD X<encode(transfer_encoding)>

=back

=item B<encoded>  X<encoded>
...
=item B<getTransferEncHandler> TYPE X<getTransferEncHandler>
