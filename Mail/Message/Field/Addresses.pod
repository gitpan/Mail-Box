...
=item B<addAddress> [ADDRESS], OPTIONS X<addAddress>
...
=item B<group> =E<gt> STRING X<addAddress(group)>
...
=item B<addGroup> GROUP|OPTIONS X<addGroup>
...
=item B<name> =E<gt> STRING X<addGroup(name)>
...
=item B<consumeAddress> STRING X<consumeAddress>
...
=item B<consumeDomain> STRING X<consumeDomain>
