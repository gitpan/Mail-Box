...
=item B<from> FIELD, OPTIONS X<from>
...
=item B<charset> =E<gt> STRING X<new(charset)>
...
=item B<is_structured> =E<gt> BOOLEAN X<new(is_structured)>
...
=item B<language> =E<gt> STRING X<new(language)>
...
=item B<addAttribute> OBJECT|(STRING, OPTIONS)|(NAME,VALUE,OPTIONS) X<addAttribute>
...
=item B<addComment> COMMENT, OPTIONS X<addComment>
...
=item B<addExtra> STRING X<addExtra>
...
=item B<addPhrase> STRING, OPTIONS X<addPhrase>
...
=item B<attributes>  X<attributes>
...
=item B<createComment> STRING, OPTIONS X<createComment>
...
=item B<createPhrase> STRING, OPTIONS X<createPhrase>
...
=item B<decodedBody> OPTIONS X<decodedBody>
...
=item B<consumeComment> STRING X<consumeComment>
...
=item B<decode> STRING, OPTIONS X<decode>
...
=item B<is_text> =E<gt> => BOOLEAN X<decode(is_text)>
...
=item B<encode> STRING, OPTIONS X<encode>
...
=item B<charset> =E<gt> STRING X<encode(charset)>
...
=item B<encoding> =E<gt> 'q'|'Q'|'b'|'B' X<encode(encoding)>
...
=item B<force> =E<gt> BOOLEAN X<encode(force)>
...
=item B<language> =E<gt> STRING X<encode(language)>
