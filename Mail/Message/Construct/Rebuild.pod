...
multiparts have to be created or removed.  The L<rebuild()|"Constructing a message"> method
...
DETAILS chapter in this manual page, and L<recursiveRebuildPart()|"Internals">.
...
messages, header field consistency, and so on.  The L<rebuild()|"Constructing a message"> method
let you focus on the task, and takes care of the rest.

=head2 Rebuilding a message

The L<rebuild()|"Constructing a message"> method uses rules to transform the one message into an
...
=head2 Description of the general rules
...
You can specify a selection of these rules with L<rebuild(rules)|"Constructing a message"> and
L<rebuild(extraRules)|"Constructing a message">.

=head2 Description of the conversion rules
