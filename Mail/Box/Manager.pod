...
=item B<default_folder_type> =E<gt> NAME|CLASS
...
=item B<folder_types> =E<gt> NEW-TYPE | ARRAY-OF-NEW-TYPES
...
=item B<folderdirs> =E<gt> [ DIRECTORY, ... ]
...
=item B<closeAllFolders> , OPTIONS
...
=item B<open> [FOLDERNAME], OPTIONS
...
=item B<type> =E<gt> FOLDERTYPENAME|FOLDERTYPE
...
=item B<registerType> TYPE, CLASS [,OPTIONS]
...
=item B<appendMessages> [FOLDER|FOLDERNAME,] MESSAGES, OPTIONS
...
=item B<copyMessage> [FOLDER|FOLDERNAME,] MESSAGES, OPTIONS
...
=item B<delete> FOLDERNAME [,OPTIONS]
...
=item B<moveMessage> [FOLDER|FOLDERNAME,] MESSAGES, OPTIONS
...
=item B<threads> [FOLDERS], OPTIONS
...
=item B<toBeThreaded> FOLDER, MESSAGES
...
=item B<toBeUnthreaded> FOLDER, MESSAGES
