...
=item B<file> =E<gt> FILEHANDLE|undef X<start(file)>
...
=item B<readHeader>  X<readHeader>
...
=item B<openFile> ARGS X<openFile>
...
=item B<filename> =E<gt> FILENAME X<openFile(filename)>

=item B<mode> =E<gt> STRING X<openFile(mode)>
