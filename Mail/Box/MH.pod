...
=item B<index> =E<gt> OBJECT X<new(index)>
...
=item B<index_filename> =E<gt> FILENAME X<new(index_filename)>
...
=item B<index_type> =E<gt> CLASS X<new(index_type)>

=item B<keep_dups> =E<gt> BOOLEAN

See Mail::Box::new(keep_dups)

=item B<keep_index> =E<gt> BOOLEAN X<new(keep_index)>
...
=item B<labels> =E<gt> OBJECT X<new(labels)>
...
=item B<labels_filename> =E<gt> FILENAME X<new(labels_filename)>
...
=item B<labels_type> =E<gt> CLASS X<new(labels_type)>
...
=item B<highestMessageNumber>  X<highestMessageNumber>
...
=item B<renumber> =E<gt> BOOLEAN X<writeMessages(renumber)>
