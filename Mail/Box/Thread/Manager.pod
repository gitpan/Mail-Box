...
=item B<folder> =E<gt> FOLDER | REF-ARRAY-FOLDERS
...
=item B<folders> =E<gt> FOLDER | REF-ARRAY-FOLDERS
...
=item B<thread_body> =E<gt> BOOLEAN
...
=item B<thread_type> =E<gt> CLASS

Type of the thread nodes.

=item B<timespan> =E<gt> TIME | 'EVER'
...
=item B<window> =E<gt> INTEGER|'ALL'
...
=item B<sortedAll> [PREPARE [COMPARE]]

Returns all() the threads by default, but sorted on timestamp.

=item B<sortedKnown> [PREPARE [,COMPARE]]
