...
L<inThread()|"Internals"> and L<thread()|"The Threads"> with a messages from the folder.
...
Returns L<all()|"The Threads"> the threads by default, but sorted on timestamp.
...
Returns all L<known()|"The Threads"> threads, in sorted order.  By default, the threads
...
threads must test with L<Mail::Message::isDummy()|"The Message"> before trying things only
...
L<Mail::Box::Manager|Mail::Box::Manager>, using L<Mail::Box::Manager::threads()|"Manage message threads">.
...
With L<all()|"The Threads"> you get the start-messages of each thread of this folder.
