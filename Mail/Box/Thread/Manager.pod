...
=item B<dummy_type> =E<gt> CLASS X<new(dummy_type)>
...
=item B<folder> =E<gt> FOLDER | REF-ARRAY-FOLDERS X<new(folder)>
...
=item B<folders> =E<gt> FOLDER | REF-ARRAY-FOLDERS X<new(folders)>
...
=item B<thread_body> =E<gt> BOOLEAN X<new(thread_body)>
...
=item B<thread_type> =E<gt> CLASS X<new(thread_type)>

Type of the thread nodes.

=item B<timespan> =E<gt> TIME | 'EVER' X<new(timespan)>
...
=item B<window> =E<gt> INTEGER|'ALL' X<new(window)>
...
=item B<folders>  X<folders>

Returns the folders as managed by this threader.

=item B<includeFolder> FOLDERS X<includeFolder>
...
=item B<removeFolder> FOLDERS X<removeFolder>
...
=item B<sortedAll> [PREPARE [COMPARE]] X<sortedAll>

Returns all() the threads by default, but sorted on timestamp.

=item B<sortedKnown> [PREPARE [,COMPARE]] X<sortedKnown>
...
=item B<thread> MESSAGE X<thread>
...
=item B<threadStart> MESSAGE X<threadStart>
...
=item B<createDummy> MESSAGE-ID X<createDummy>
...
=item B<inThread> MESSAGE X<inThread>
...
=item B<outThread> MESSAGE X<outThread>
