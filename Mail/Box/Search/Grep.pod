...
=item B<deliver> =E<gt> undef|CODE|'DELETE'|LABEL|'PRINT'|REF-ARRAY
...
=item B<field> =E<gt> undef|STRING|REGEX|CODE
...
=item B<match> =E<gt> STRING|REGEX|CODE
...
=item B<printMatchedBody> FILEHANDLE, MATCH

=item B<printMatchedHead> FILEHANDLE, MATCH
