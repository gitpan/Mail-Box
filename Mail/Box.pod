...
=item B<delete_copied> =E<gt> BOOLEAN
...
=item B<select> =E<gt> 'ACTIVE'|'DELETED'|'ALL'|LABEL|!LABEL|FILTER

Which messages are to be copied. See messages(description)
about how this works.

=item B<subfolders> =E<gt> BOOLEAN|'FLATTEN'|'RECURSE'
...
=item B<write> =E<gt> 'ALWAYS'|'NEVER'|'MODIFIED'
