...
=item B<askSubfolderSeparator>  X<askSubfolderSeparator>

Returns the separator which is used on the server side to indicate
sub-folders.

=item B<askSubfoldersOf> NAME X<askSubfoldersOf>

Returns a list of subfolders for this server.

=item B<getFlag> ID, LABEL X<getFlag>
...
=item B<setFlags> ID, LABEL, VALUE, [LABEL, VALUE], ... X<setFlags>
